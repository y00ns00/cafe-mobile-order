package ys.member.common;

import org.springframework.http.HttpStatus;
import org.springframework.stereotype.Component;
import ys.member.exception.errorcode.MemberDomainErrorCode;
import ys.member.exception.errorcode.MemberErrorCode;

/**
 * ErrorCode를 HttpStatus로 매핑하는 전략 클래스
 * Domain/Exception 계층의 Web 의존성 제거를 위해 Web 계층에서 매핑 담당
 */
@Component
public class ErrorCodeHttpStatusMapper {

    /**
     * ErrorCode에 따라 적절한 HttpStatus를 매핑
     *
     * @param errorCode 에러 코드
     * @return 매핑된 HTTP 상태 코드
     */
    public HttpStatus resolve(MemberErrorCode errorCode) {
        // MemberDomainErrorCode의 경우 코드에 따라 다른 상태 코드 반환
        if (errorCode instanceof MemberDomainErrorCode domainErrorCode) {
            return switch (domainErrorCode) {
                case MEMBER_NOT_FOUND -> HttpStatus.NOT_FOUND;
                case DUPLICATE_MEMBER -> HttpStatus.CONFLICT;
                default -> HttpStatus.BAD_REQUEST;
            };
        }

        // MemberValidationErrorCode는 모두 BAD_REQUEST
        return HttpStatus.BAD_REQUEST;
    }
}